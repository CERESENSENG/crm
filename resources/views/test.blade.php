<x-layout>
  ...
  <form method="POST" action="{{ route('student.storecsv') }}" enctype="multipart/form-data" class="needs-validation"
      novalidate="">
      @csrf
      ...
      <table class="table">
          ...
          @foreach ($confirms as $index => $confirm)
              <tr>
                  ...
                  <td>{!! $confirm->error_in_matric ? '<span class="text-danger" >' . $confirm->appNo . '</span>' : $confirm->matricNo !!}
              ...
              {{--                   
              <input   type="hidden"     value="{{$storedeptId->id }}"  >   --}}
